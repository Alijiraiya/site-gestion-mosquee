...
/* --------------------------------------------------------------------------
   ...
   Avant, sync-terminal.txt dependait du shell (Tee-Object). Des que la sortie
   n'etait pas redirigee, ou apres un aller-retour par git, le fichier etait
   absent. La suite ecrit desormais elle-meme son journal, sans codes ANSI.
   Option : --log mon-journal.txt
   -------------------------------------------------------------------------- */

const TERM_LOG = String(arg("log", "sync-terminal.txt"));
const ANSI_RE = /\u001b\[[0-9;]*m/g;
let TERM_BUF = [];
const UTF8_BOM = "\uFEFF";
try {
  fs.writeFileSync(TERM_LOG, UTF8_BOM, "utf8");
} catch {
  /* disque en lecture seule : on continue sans journal */
}
function termFlush() {
  ...
  TERM_BUF = [];
  try {
    fs.appendFileSync(TERM_LOG, chunk, "utf8");
  } catch {
    /* ignore */
  }
}
...
test("T3", "Modification du chef de famille : miroir vers la famille et recalcul SVF", async () => {
  ...
  // L'API renvoie un OBJET decrivant les champs reportes, pas un booleen.
  const mir1 = up.data?.headMirrored;
  check(
    mir1 !== null && typeof mir1 === "object" && "dateOfBirth" in mir1,
...
test("T4", "Modification de la fiche famille : le chef est mis \u00e0 jour en retour", async () => {
  ...
  const mir2 = up.data?.headMirrored;
  check(
    mir2 !== null && typeof mir2 === "object" && !Array.isArray(mir2),
...
test("T5", "Suppression d'un membre : effectif et score recalcul\u00e9s", async () => {
  ...
  // "OTHER" fait partie de l'enum FamilyMemberRole (HEAD/SPOUSE/CHILD/PARENT/OTHER).
  const add = await addMember(fam.id, { role: "OTHER", dateOfBirth: dobForAge(20) });
...
async function main() {
  ...
  fs.writeFileSync(`${CFG.out}.md`, UTF8_BOM + buildMarkdown(json), "utf8");
  ...
  termFlush();
  console.log(c("\n  FICHIERS ECRITS -- l-heure doit correspondre a maintenant"));
  for (const fp of [`${CFG.out}.json`, `${CFG.out}.md`, TERM_LOG]) {
    try {
      const st = fs.statSync(fp);
      console.log(z(`    ${String(st.size).padStart(9)} o   ${st.mtime.toLocaleString()}   ${fp}`));
    } catch (e) {
      console.log(r(`    ABSENT   ${fp}   (${String(e?.message || e)})`));
    }
  }
  console.log(z(`    Dossier courant : ${process.cwd()}`));
