 ...
 *
 *  COUVERTURE (16 sections, ~600+ assertions) :
 ...
 *
 *  SORTIES : test-report-full.json  +  test-report-full.md   (dans le dossier courant)
...
function oracleSVF(p) {
  ...
  if (p.noSupport) { s += CALW("points_no_support"); parts.push(["sans soutien", CALW("points_no_support")]) }
  ...
  if (Number.isFinite(age)) {
    ...
    else if (age <= W("young_head_age_threshold")) { s += CALW("points_young_head"); parts.push(["jeune chef", CALW("points_young_head")]) }
  }
...
section("S1", "Authentification (login, profil, mot de passe, sessions)", async () => {
  ...
  await guard("S1.6", async () => {
    ...
    expectFailure("S1.6.1 newPassword sans currentPassword",
      await PUT("/api/auth", { newPassword: NEW }), 400, "currentPassword is required to change the password.")
    expectFailure("S1.6.2 currentPassword incorrect",
      await PUT("/api/auth", { currentPassword: "totalement-faux-999", newPassword: NEW }), 401, "Current password is incorrect.")
    expectFailure("S1.6.3 nouveau mot de passe trop court (7)",
      await PUT("/api/auth", { currentPassword: String(CFG.password), newPassword: "1234567" }), 400, "New password must be at least 8 characters.")
    expectFailure("S1.6.4 nouveau mot de passe vide",
      await PUT("/api/auth", { currentPassword: String(CFG.password), newPassword: "" }), 400, null)

    /* Limite exacte : 8 caracteres doit passer */
    const r8 = await PUT("/api/auth", { currentPassword: String(CFG.password), newPassword: "Zz8chars" })
    if (expectOk("S1.6.5 8 caracteres accepte (limite exacte)", r8, 200)) {
      ...
      const r2 = await PUT("/api/auth", { currentPassword: "Zz8chars", newPassword: NEW })
      ...
      const rBack = await PUT("/api/auth", { currentPassword: NEW, newPassword: String(CFG.password) })
...
section("S2", "Creation de compte + mosquee (register)", async () => {
  ...
  await guard("S2.1", async () => {
    ...
    expectFailure("S2.1.3 objet mosque manquant", await POST("/api/auth/register", bNoMosque, { token: null }), 400, "Missing required registration fields.")
...
section("S4", "Parametres : matrices de validation, poids SVF, reset, propagation", async () => {
  ...
  await guard("S4.1", async () => {
    ...
    const missing = expectedWeightKeys.filter((k) => !(k in w))
  ...
  await guard("S4.3", async () => {
    for (const f of INT_FIELDS) {
      ...
      expectFailure(`S4.3.4 ${f} negatif refuse`, await PUT("/api/settings", { [f]: -1 }), 400, `${f} cannot be negative.`)
  ...
  await guard("S4.4", async () => {
    for (const f of NUMBER_FIELDS) {
      ...
      expectFailure(`S4.4.2 ${f} negatif refuse`, await PUT("/api/settings", { [f]: -0.5 }), 400, `${f} cannot be negative.`)
  ...
  await guard("S4.5", async () => {
    ...
    expectFailure("S4.5.5 reservePercentage negatif", await PUT("/api/settings", { reservePercentage: -5 }), 400, "reservePercentage cannot be negative.")
  ...
  await guard("S4.8", async () => {
    const r = await PUT("/api/settings", { svfWeights: { points_per_child: 6, points_widow_divorced: 17 } })
    if (expectOk("S4.8.1 PUT svfWeights partiel", r, 200)) {
      eq("S4.8.2 points_per_child = 6", num(r.data.svfWeights.points_per_child), 6)
      eq("S4.8.3 points_widow_divorced = 17", num(r.data.svfWeights.points_widow_divorced), 17)
      check("S4.8.4 les autres poids sont conserves (fusion, pas remplacement)",
        Object.keys(SVF_FALLBACK).every((k) => k in r.data.svfWeights), Object.keys(r.data.svfWeights).length)
    ...
    const rJunk = await PUT("/api/settings", { svfWeights: { cle_bidon: 99, points_per_child: "abc", autre: null } })
    ...
    if (rJunk.status === 200) {
      ...
      check("S4.8.9 points_per_child='abc' n'a pas corrompu le poids",
        typeof rJunk.data.svfWeights.points_per_child === "number" && Number.isFinite(rJunk.data.svfWeights.points_per_child),
        rJunk.data.svfWeights.points_per_child)
    }
    ...
    if (rCol.status === 200) {
      ...
      eq("S4.8.11 colonne pointsPerChild se reflete dans svfWeights.points_per_child", num(w.points_per_child), 9)
    }
  })
  ...
  await guard("S4.9", async () => {
    ...
    const rUp = await PUT("/api/settings", { svfWeights: { points_widow_divorced: 30 } })
    ...
    const rDown = await PUT("/api/settings", { svfWeights: { points_widow_divorced: 0 } })
    if (rDown.status === 200) {
    ...
    const back = await PUT("/api/settings", { svfWeights: { points_widow_divorced: W("points_widow_divorced") } })
    if (back.status === 200) {
    ...
    if (fk) {
      ...
      for (const pw of [2, 4, 6, 8, 10]) {
        const rr = await PUT("/api/settings", { svfWeights: { points_per_child: pw } })
      ...
      await PUT("/api/settings", { svfWeights: { points_per_child: W("points_per_child") } })
    }
  })
  ...
  await guard("S4.10", async () => {
    ...
    expectFailure("S4.10.2 scope manquant", await POST("/api/settings/reset", {}), 400, 'scope must be "svf", "water-filling" or "all".')
    expectFailure("S4.10.3 scope numerique", await POST("/api/settings/reset", { scope: 1 }), 400, 'scope must be "svf", "water-filling" or "all".')

    await PUT("/api/settings", { svfWeights: { points_per_child: 13, points_disability: 33 } })
    ...
    if (expectOk("S4.10.4 reset scope=svf", rSvf, 200)) {
      ...
      eq("S4.10.5 points_per_child revenu a 5", num(w.points_per_child), 5)
      eq("S4.10.6 points_disability revenu a 15", num(w.points_disability), 15)
      ...
      eq("S4.10.12 svfCustomized repasse a false", rSvf.data.svfCustomized, false)
    }
    ...
    if (expectOk("S4.10.13 reset scope=water-filling", rWf, 200)) {
      ...
      eq("S4.10.16 reset water-filling ne touche pas les poids SVF", num(w.points_per_child), 5)
    }

    await PUT("/api/settings", { svfWeights: { points_per_child: 11 }, reservePercentage: 0.33 })
    ...
    if (expectOk("S4.10.17 reset scope=all", rAll, 200)) {
      ...
      eq("S4.10.18 poids enfant remis a 5", num(w.points_per_child), 5)
    ...
    STATE.weights = fin.w || STATE.weights
...
section("S5", "Familles : creation, validations, logement, filtres, edition, suppression", async () => {
  ...
  await guard("S5.9", async () => {
    ...
    const editable = {
      ...
      wilaya: "Alger", commune: "Bab Ezzouar", monthlyIncome: 9000, notes: "note de test",
    }
...
section("S6", "Membres et enfants : roles, garde-fous HEAD, promotion SPOUSE, compteurs", async () => {
  ...
  await guard("S6.3", async () => {
    ...
    expectFailure("S6.3.1 ajout d'un 2e HEAD -> 409", rSecond, 409, "A family can only have one head.")

    const { m: other } = await addMember(f.id, { firstName: "Autre", role: "OTHER" })
    if (other) {
      const rPromote = await PUT(`/api/members/${other.id}`, { role: "HEAD" })
      expectFailure("S6.3.2 promotion d'un membre en HEAD -> 409", rPromote, 409, "A family can only have one head.")
    }
    if (head) {
    ...
    if (f2) {
      ...
      check("S6.3.5 ajout d'un HEAD dans une famille sans chef accepte", rAddHead.status < 300, { status: rAddHead.status, message: rAddHead.message })
    }
  })
  ...
  await guard("S6.8", async () => {
    ...
    const lst = await GET(`/api/families/${f.id}/members`)
    eq("S6.8.3 la base contient bien les 10 lignes", ((lst.data || {}).members || []).length, N)
    const { f: af } = await getFamily(f.id)
...
section("S7", "SVF : calibration de chaque facteur, profils composites, mutations", async () => {
  ...
  await guard("S7.3", async () => {
    ...
    const ages = [
      ...
      { a: yng, attendu: CALW("points_young_head"), label: `${yng} ans (borne jeune)` },
  ...
  await guard("S7.6", async () => {
    ...
    check("S7.6.101 membersCount a bien progresse avec les ajouts", num(finale.membersCount) >= 8, { membersCount: finale.membersCount })
  ...
  await guard("S11.2", async () => {
    ...
    if (dnId) {
      ...
      if (rc.status < 300) {
        ...
        const after = num((((await GET("/api/dashboard/financial-summary")).data || {}).summary || {}).totalIn)
...
section("S12", "Documents joints aux familles", async () => {
  await guard("S12.1", async () => {
    ...
    if (docId) {
      ...
      check("S12.1.12 en-tete de contenu present", !!(dl.headers && (dl.headers["content-type"] || dl.headers["content-disposition"])), dl.headers)
...
section("S13", "Securite : 401 systematique, jetons falsifies, cloisonnement multi-mosquees", async () => {
  ...
  await guard("S13.3", async () => {
    ...
    if (rSetB.status === 200) {
      ...
      check("S13.3.11 modifier les parametres de B n'affecte pas ceux de A", num(mine.settings.pointsPerChild) !== 3 || W("points_per_child") !== 3, { A: mine.settings.pointsPerChild })
    }
  })
...
section("S14", "Contrats d'API : enveloppe success/data, types, methodes, en-tetes", async () => {
  ...
  await guard("S14.1", async () => {
    ...
    for (const p of routes) {
      ...
      check(`S14.1.4 ${p} : en-tete content-type JSON`, /application\/json/i.test(String((r.headers || {})["content-type"] || "")), (r.headers || {})["content-type"])
...
function buildJson(stats, dureeMs) {
  ...
  return {
    ...
    fatal: FATAL,
    resultats: RESULTS,
    echecs: RESULTS.filter((r) => r.type === "FAIL"),
  }
}
...
async function main() {
    ...
    if (!boot) {
      ...
      for (let iter = 1; iter <= CFG.repeat; iter++) {
        ...
        for (const s of SECTIONS) {
          ...
          const d = Date.now() - st
          const mine = RESULTS.filter((r) => r.section === s.id && r.iteration === iter)
          const p = mine.filter((r) => r.type === "PASS").length
          const f = mine.filter((r) => r.type === "FAIL").length
  ...
  try {
    ...
    fs.writeFileSync(mdPath, buildMarkdown(j), "utf8")
  } catch (e) {
